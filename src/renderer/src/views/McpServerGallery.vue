  ...
  <div class="w-full h-full flex flex-col bg-background">
    ...
    <div class="flex items-center justify-between p-4 border-b">
      <div class="flex items-center gap-3">
        ...
        <h1 class="text-2xl font-semibold">{{ t('mcpGallery.title') }}</h1>
      </div>
    ...
    <div class="flex items-center gap-4 p-4 border-b">
      <div class="flex-1 relative">
        ...
        <Input
          v-model="searchQuery"
          :placeholder="t('mcpGallery.searchPlaceholder')"
          class="pl-10"
        />
      </div>
      <Select v-model="filterStatus">
        <SelectTrigger class="w-48">
          <SelectValue :placeholder="t('mcpGallery.filterByStatus')" />
        </SelectTrigger>
        <SelectContent>
          <SelectItem value="all">{{ t('mcpGallery.allServers') }}</SelectItem>
          <SelectItem value="running">{{ t('mcpGallery.runningServers') }}</SelectItem>
          <SelectItem value="stopped">{{ t('mcpGallery.stoppedServers') }}</SelectItem>
          <SelectItem value="error">{{ t('mcpGallery.errorServers') }}</SelectItem>
        </SelectContent>
      </Select>
    </div>
    ...
    <div class="flex-1 overflow-auto p-4">
                ...
                <DropdownMenuContent align="end">
                  ...
                  <DropdownMenuItem @click="toggleServer(server)">
                    <Icon
                      ...
                      class="h-4 w-4 mr-2"
                    />
                    {{ server.isRunning ? t('mcpGallery.stopServer') : t('mcpGallery.startServer') }}
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center space-x-4 text-xs text-muted-foreground">
                  <span class="flex items-center gap-1">
                    ...
                    {{ server.toolsCount }} {{ t('mcpGallery.tools') }}
                  </span>
                  <span class="flex items-center gap-1">
                    <Icon icon="lucide:message-square-quote" class="w-3 h-3" />
                    {{ server.promptsCount }} {{ t('mcpGallery.prompts') }}
                  </span>
                  <span class="flex items-center gap-1">
                    <Icon icon="lucide:folder" class="w-3 h-3" />
                    {{ server.resourcesCount }} {{ t('mcpGallery.resources') }}
                  </span>
                </div>
              </div>
              <div class="flex items-center gap-3">
      ...
      <div v-if="filteredServers.length === 0" class="flex flex-col items-center justify-center h-64 text-center">
        ...
        <h3 class="text-lg font-medium mb-2">{{ t('mcpGallery.noServers') }}</h3>
        <p class="text-muted-foreground mb-4">{{ t('mcpGallery.noServersDescription') }}</p>
        <Button @click="showAddDialog = true" class="gap-2">
          <Icon icon="lucide:plus" class="w-4 h-4" />
          {{ t('mcpGallery.addFirstServer') }}
        </Button>
      </div>
    </div>
  </div>
...
const getStatusText = (status: string) => {
  switch (status) {
    case 'running':
      return t('mcpGallery.running')
    case 'loading':
      return t('mcpGallery.starting')
    case 'error':
      return t('mcpGallery.error')
    default:
      return t('mcpGallery.stopped')
  }
}
